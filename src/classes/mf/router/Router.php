...
class Router extends AbstractRouter
{
  ...
  public function addRoute(string $name, string $action, string $ctrl, int $acces_level=100): void
  {
  ...
  public function run(): void
  {
    if (!isset($this->request->get['action'])) {
      $recupAction=self::$aliases['default'];
      $ctrl=new self::$routes[$recupAction][0]();
      $ctrl->execute();
    } else {
        $routes=$this->request->get['action'];
        if (isset(self::$routes[$routes]) && TweeterAuthentification::checkAccessRight(self::$routes[$routes][1])) {
          $ctrl = new self::$routes[$routes][0]();
          $ctrl->execute();
        }
        else {
          $recupAction=self::$aliases['default'];
  ...
  static public function executeRoute(string $alias) : void{
    $recupAction = self::$aliases[$alias];
    $action = new self::$routes[$recupAction][0];
    $action->execute();
  }
